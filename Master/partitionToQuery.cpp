...
//得到子查询计划
vector<structPlan> partitionToSub:: getSubPlan() const{
    
    return subPlan;
}

//
subQuery* partitionToSub:: getSubQuery(size_t id) const{
    
    auto it = subRef.find(id);
    if(it!= subRef.end()){
...
//得到子查询变量
vector<string> partitionToSub::getSubQueryVec() const{
    
    return subQueryVec;
}

//得到子查询变量个数
size_t partitionToSub::getSubQueryVecSize() const{
...
//根据ID得到子查询语句
vector<string> partitionToSub::getSubQueryStr(size_t id) const{
    
    auto it = subRef.find(id);
    if(it == subRef.end()){
        exit(0);
    }
    subQuery* temp = it->second;
    
    return temp->getQueryVec();
}

//根据ID得到变量名
vector<string> partitionToSub::getSubQueryName(size_t id) const{
...
//得到查询计划数组元素大小
size_t partitionToSub::getSubPlanSize() const{
