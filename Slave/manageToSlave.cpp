        ...
        for(auto it = coList.begin(); it != coList.end(); it++){
            ...
            if(retval == -1){
                ...
                if(str == "create"){
                    ...
                    //创建查询类
                    unordered_map<size_t, string> sub1;
                    unordered_map<size_t, vector<string> > nameU;
                  ...
                  // ID type
                    ...
                    cout<<"接收查询计划完成，下面开始执行查询计划"<<endl;
                    //传入查询计划并执行
                    if(!plan1.empty()){
                    cout<<"查询计划非空，开始执行查询计划"<<endl;
                    ...
                    queryClass* qc =  temp->getSubQueryClass(idRe);
...
queryClass* manageToSlave::getSubQuery(size_t id1, size_t id2){
    ...
    if(it == umap_Gen_Query.end()){
        ...
        if(temp1->getID() == 0){
            ...
            if(nodeNum == ID){//s属于该节点但是还未完成，等待
                while(1){
                    queryClass* temp2 = temp->getSubQueryClass(id2);
                    if(temp2->getID() == 0) delete temp2;
                    else {
                        return temp2;
                    }
                    usleep(50);
