...
Status OneConstantStatisticsBuffer::save(MMapBuffer *&indexBuffer, int slaveID) {
#ifdef DEBUG
	...
	if (indexBuffer == NULL) {
		indexBuffer = MMapBuffer::create(
				string(string(DATABASE_PATH) + "/statIndex-" + std::to_string(slaveID)).c_str(),
				(index.size() + 2) * 4);
	...
	cerr << "indexBuffer->getSize(): " << indexBuffer->getSize() << endl;
...
OneConstantStatisticsBuffer *OneConstantStatisticsBuffer::load(StatisticsType type,
                                                               const string path,
                                                               char *&indexBuffer) {
...
Status TwoConstantStatisticsBuffer::save(MMapBuffer *&indexBuffer, int slaveID) {
	char *writer;
	if (indexBuffer == NULL) {
		indexBuffer = MMapBuffer::create(
				string(string(DATABASE_PATH) + "/statIndex-" + std::to_string(slaveID)).c_str(),
	...
	cerr << "TwoConstantStatisticsBuffer::save-indexBuffer->getSize(): " << indexBuffer->getSize() << endl;
	writer = writeData(writer, usedSpace);
...
TwoConstantStatisticsBuffer *TwoConstantStatisticsBuffer::load(StatisticsType type,
                                                               const string path,
                                                               char *&indexBuffer) {
