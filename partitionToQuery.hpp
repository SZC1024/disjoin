...
public:
    ...
    vector<structPlan> getSubPlan();     //得到查询计划
    
    size_t getSubPlanSize();   //得到查询计划元素个数
    
    subQuery* getSubQuery(size_t id);    //根据ID得到某子查询
    
    vector<string> getSubQueryVec();    //得到查询语句
    
    size_t getSubQueryVecSize();     //得到查询语句个数
    
    vector<size_t> getAllSubID();   //得到分区所有子查询ID
    
    vector<string> getSubQueryStr(size_t id); //根据ID得到它的子查询
    
    vector<string> getSubQueryName(size_t id);  //根据ID得到子查询变量名
