...
Status PredicateTable::insertTable(const char *str, ID &id) {
	...
	if (!prefix_segment->findIdByString(prefixId, &prefix))
...
string PredicateTable::getPrediacateByID(ID id) {
	searchStr.clear();
	if (!suffix_segment->findStringById(&suffix, id))
		return searchStr;
	char temp[10];
	memset(temp, 0, 10);
	...
	if (prefixId == 1)
		...
		if (!prefix_segment->findStringById(&prefix, prefixId))
			return string("");
...
Status PredicateTable::getIDByPredicate(const char *str, ID &id) {
	...
	if (prefix.equals(SINGLE.c_str())) {
		...
		searchLen.str = searchStr.c_str();
		searchLen.length = searchStr.length();
		if (!suffix_segment->findIdByString(id, &searchLen))
